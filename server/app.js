...
function authenticateToken(req, res, next) {
  ...
  if (!token) return res.status(401).json({ error: 'رمز التوثيق مطلوب' });
  try {
    const payload = jwt.verify(token, JWT_SECRET);
    if (payload.twoFactorPending) return res.status(403).json({ error: 'جلسة التوثيق غير كاملة' });
    req.user = payload; next();
  } catch { return res.status(403).json({ error: 'رمز توثيق غير صالحة' }); }
}
...
function authenticatePendingToken(req, res, next) {
  ...
  try {
    const payload = jwt.verify(token, JWT_SECRET);
    if (!payload.twoFactorPending) return res.status(403).json({ error: 'رمز 2FA غير صالح' });
    req.user = payload; next();
  } catch { return res.status(403).json({ error: 'رمز غير صالح' }); }
}

function requireAdmin(req, res, next) {
  if (req.user?.role !== 'admin') return res.status(403).json({ error: 'صلاحيات مسؤولة مطلوبة' });
  next();
}
...
function requireCoordinatorOrAdmin(req, res, next) {
  if (!['admin', 'coordinator'].includes(req.user?.role)) {
    return res.status(403).json({ error: 'صلاحيات منسق أو مسؤولة مطلوبة' });
  }
  next();
}

function requireAdminOrInspectionCoordinator(req, res, next) {
  if (!['admin', 'inspection_coordinator'].includes(req.user?.role)) {
    return res.status(403).json({ error: 'صلاحيات مسؤولة أو منسق فحص مطلوبة' });
  }
  next();
}
...
async function activeSession() {
  return await db.prepare("SELECT * FROM sessions WHERE status = 'active' ORDER BY id DESC LIMIT 1").get();
}
...
app.post('/api/sessions', authenticateToken, requireAdmin, async (req, res) => {
  ...
  await db.prepare("UPDATE sessions SET status='closed', end_date=CURRENT_TIMESTAMP WHERE status='active'").run();
  const result = await db.prepare(
    "INSERT INTO sessions (name, status, notes, created_by) VALUES (?, 'active', ?, ?)"
  ).run(name, notes || '', req.user.id);
  const created = await db.prepare('SELECT * FROM sessions WHERE id = ?').get(result.lastInsertRowid);
  res.status(201).json(created);
});

app.post('/api/sessions/:id/close', authenticateToken, requireAdmin, async (req, res) => {
  ...
  await db.prepare("UPDATE sessions SET status='closed', end_date=CURRENT_TIMESTAMP, notes=? WHERE id=?")
    .run(notes ?? session.notes, req.params.id);
  res.json(await db.prepare('SELECT * FROM sessions WHERE id = ?').get(req.params.id));
});

app.patch('/api/sessions/:id', authenticateToken, requireAdmin, async (req, res) => {
  const { notes } = req.body;
  await db.prepare('UPDATE sessions SET notes=? WHERE id=?').run(notes ?? '', req.params.id);
  res.json(await db.prepare('SELECT * FROM sessions WHERE id = ?').get(req.params.id));
});
...
app.get('/api/categories', authenticateToken, async (req, res) => {
  res.json(await db.prepare('SELECT * FROM categories WHERE is_active = 1 ORDER BY id').all());
});

app.post('/api/categories', authenticateToken, requireAdmin, async (req, res) => {
  const { name, unit_price, unit_type } = req.body;
  const result = await db.prepare('INSERT INTO categories (name, unit_price, unit_type, is_active) VALUES (?, ?, ?, 1)')
...
app.put('/api/categories/:id', authenticateToken, requireAdmin, async (req, res) => {
  ...
  await db.prepare('UPDATE categories SET name = ?, unit_price = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
    .run(name ?? category.name, Number(unit_price ?? category.unit_price), req.params.id);
  res.json(await db.prepare('SELECT * FROM categories WHERE id = ?').get(req.params.id));
});

/* ═══════════════════════════════════════════
   CATEGORY USAGE
═══════════════════════════════════════════ */
app.get('/api/category-usage', authenticateToken, async (req, res) => {
  const { date, session_id } = req.query;
  let rows;
  if (date)
    rows = await db.prepare('SELECT * FROM category_usage WHERE entry_date = ? AND approved = 1 ORDER BY id').all(date);
  else if (session_id)
    rows = await db.prepare('SELECT * FROM category_usage WHERE session_id = ? AND approved = 1 ORDER BY entry_date DESC, id DESC').all(session_id);
  else
    rows = await db.prepare('SELECT * FROM category_usage WHERE approved = 1 ORDER BY entry_date DESC, id DESC').all();
  res.json(rows);
});

app.post('/api/category-usage', authenticateToken, async (req, res) => {
  ...
  res.status(201).json(await db.prepare('SELECT * FROM category_usage WHERE id = ?').get(result.lastInsertRowid));
});

app.delete('/api/category-usage/:id', authenticateToken, requireAdmin, async (req, res) => {
  const row = await db.prepare('SELECT * FROM category_usage WHERE id = ?').get(req.params.id);
  if (!row) return res.status(404).json({ error: 'السجل غير موجود' });
  await db.prepare('DELETE FROM category_usage WHERE id = ?').run(req.params.id);
...
/* ═══════════════════════════════════════════
   HOURLY WORKERS
═══════════════════════════════════════════ */
app.get('/api/hourly-workers', authenticateToken, async (req, res) => {
  res.json(await db.prepare('SELECT * FROM hourly_workers WHERE is_active = 1 ORDER BY id').all());
});

app.post('/api/hourly-workers', authenticateToken, requireAdmin, async (req, res) => {
  const { full_name, hourly_rate, notes } = req.body;
  const result = await db.prepare('INSERT INTO hourly_workers (full_name, hourly_rate, notes, is_active) VALUES (?, ?, ?, 1)')
    .run(full_name, Number(hourly_rate || 0), notes || '');
  res.status(201).json(await db.prepare('SELECT * FROM hourly_workers WHERE id = ?').get(result.lastInsertRowid));
});

app.put('/api/hourly-workers/:id', authenticateToken, requireAdmin, async (req, res) => {
  const { full_name, hourly_rate, notes } = req.body;
  const worker = await db.prepare('SELECT * FROM hourly_workers WHERE id = ?').get(req.params.id);
  if (!worker) return res.status(404).json({ error: 'العامل غير موجود' });
  await db.prepare('UPDATE hourly_workers SET full_name=?, hourly_rate=?, notes=? WHERE id=?')
    .run(full_name ?? worker.full_name, Number(hourly_rate ?? worker.hourly_rate), notes ?? worker.notes, req.params.id);
  res.json(await db.prepare('SELECT * FROM hourly_workers WHERE id = ?').get(req.params.id));
});

/* ═══════════════════════════════════════════
   SHIFT RECORDS
═══════════════════════════════════════════ */
app.get('/api/shift-records', authenticateToken, async (req, res) => {
  const { date, session_id } = req.query;
  let rows;
  const base = 'SELECT sr.*, hw.full_name as worker_name FROM shift_records sr JOIN hourly_workers hw ON sr.worker_id = hw.id';
  if (date)
    rows = await db.prepare(`${base} WHERE sr.shift_date = ? ORDER BY sr.id`).all(date);
  else if (session_id)
    rows = await db.prepare(`${base} WHERE sr.session_id = ? ORDER BY sr.shift_date DESC, sr.id DESC`).all(session_id);
  else
    rows = await db.prepare(`${base} ORDER BY sr.shift_date DESC, sr.id DESC`).all();
  res.json(rows);
});

app.post('/api/shift-records', authenticateToken, requireAdmin, async (req, res) => {
  const { worker_id, shift_date, shift_start, shift_end } = req.body;
  const worker = await db.prepare('SELECT * FROM hourly_workers WHERE id = ?').get(worker_id);
  if (!worker) return res.status(404).json({ error: 'العامل غير موجود' });
  ...
  res.status(201).json(await db.prepare('SELECT * FROM shift_records WHERE id = ?').get(result.lastInsertRowid));
});

app.put('/api/shift-records/:id', authenticateToken, requireAdmin, async (req, res) => {
  const { shift_start, shift_end } = req.body;
  const row = await db.prepare('SELECT * FROM shift_records WHERE id = ?').get(req.params.id);
  if (!row) return res.status(404).json({ error: 'السجل غير موجود' });
  ...
  await db.prepare('UPDATE shift_records SET shift_start = ?, shift_end = ?, hours_worked = ?, total_pay = ? WHERE id = ?')
    .run(start, end, Number(hoursWorked.toFixed(2)), total_pay, req.params.id);
  res.json(await db.prepare('SELECT * FROM shift_records WHERE id = ?').get(req.params.id));
});

app.delete('/api/shift-records/:id', authenticateToken, requireAdmin, async (req, res) => {
  const row = await db.prepare('SELECT * FROM shift_records WHERE id = ?').get(req.params.id);
  if (!row) return res.status(404).json({ error: 'السجل غير موجود' });
  await db.prepare('DELETE FROM shift_records WHERE id = ?').run(req.params.id);
  res.json({ success: true });
});
...
═══════════════════════════════════════════ */
app.get('/api/box-allocations', authenticateToken, async (req, res) => {
  const { date, session_id } = req.query;
  const base = 'SELECT ba.*, hw.full_name as worker_name FROM box_allocations ba JOIN hourly_workers hw ON ba.worker_id = hw.id';
  let rows;
  if (date)
    rows = await db.prepare(`${base} WHERE ba.alloc_date = ? AND ba.approved = 1 ORDER BY ba.id`).all(date);
  else if (session_id)
    rows = await db.prepare(`${base} WHERE ba.session_id = ? AND ba.approved = 1 ORDER BY ba.alloc_date DESC, ba.id DESC`).all(session_id);
  else
    rows = await db.prepare(`${base} WHERE ba.approved = 1 ORDER BY ba.alloc_date DESC, ba.id DESC`).all();
  res.json(rows);
});

app.post('/api/box-allocations', authenticateToken, requireCoordinatorOrAdmin, async (req, res) => {
  ...
  const session = await activeSession();
  const approved = req.user.role === 'admin' ? 1 : 0;
  const result = await db.prepare('INSERT INTO box_allocations (worker_id, boxes_count, alloc_date, session_id, approved, created_by) VALUES (?,?,?,?,?,?)')
    .run(worker_id, Number(boxes_count || 0), date, session?.id ?? null, approved, req.user.id);
  const created = await db.prepare('SELECT ba.*, hw.full_name as worker_name FROM box_allocations ba JOIN hourly_workers hw ON ba.worker_id = hw.id WHERE ba.id = ?').get(result.lastInsertRowid);
  res.status(201).json(created);
});

app.put('/api/box-allocations/:id', authenticateToken, requireCoordinatorOrAdmin, async (req, res) => {
  const { worker_id, boxes_count } = req.body;
  const row = await db.prepare('SELECT * FROM box_allocations WHERE id = ?').get(req.params.id);
  if (!row) return res.status(404).json({ error: 'السجل غير موجود' });
  const worker = await db.prepare('SELECT * FROM hourly_workers WHERE id = ?').get(worker_id);
  if (!worker) return res.status(404).json({ error: 'العامل غير موجود' });
  await db.prepare('UPDATE box_allocations SET worker_id=?, boxes_count=? WHERE id=?')
    .run(worker_id, Number(boxes_count || 0), req.params.id);
  const updated = await db.prepare('SELECT ba.*, hw.full_name as worker_name FROM box_allocations ba JOIN hourly_workers hw ON ba.worker_id = hw.id WHERE ba.id = ?').get(req.params.id);
  res.json(updated);
});

app.delete('/api/box-allocations/:id', authenticateToken, requireCoordinatorOrAdmin, async (req, res) => {
  const row = await db.prepare('SELECT * FROM box_allocations WHERE id = ?').get(req.params.id);
  if (!row) return res.status(404).json({ error: 'السجل غير موجود' });
  await db.prepare('DELETE FROM box_allocations WHERE id = ?').run(req.params.id);
  res.json({ success: true });
});
...
═══════════════════════════════════════════ */
app.get('/api/daily-workers', authenticateToken, async (req, res) => {
  res.json(await db.prepare('SELECT * FROM daily_workers WHERE is_active = 1 ORDER BY id').all());
});

app.post('/api/daily-workers', authenticateToken, requireAdmin, async (req, res) => {
  const { full_name, daily_rate, notes } = req.body;
  if (!full_name) return res.status(400).json({ error: 'الاسم مطلوب' });
  const result = await db.prepare('INSERT INTO daily_workers (full_name, daily_rate, notes, is_active) VALUES (?,?,?,1)')
    .run(full_name, Number(daily_rate || 0), notes || '');
  res.status(201).json(await db.prepare('SELECT * FROM daily_workers WHERE id = ?').get(result.lastInsertRowid));
});

app.put('/api/daily-workers/:id', authenticateToken, requireAdmin, async (req, res) => {
  const { full_name, daily_rate, notes, is_active } = req.body;
  const worker = await db.prepare('SELECT * FROM daily_workers WHERE id = ?').get(req.params.id);
  if (!worker) return res.status(404).json({ error: 'العامل غير موجود' });
  await db.prepare('UPDATE daily_workers SET full_name=?, daily_rate=?, notes=?, is_active=? WHERE id=?')
    .run(
  ...
  res.json(await db.prepare('SELECT * FROM daily_workers WHERE id = ?').get(req.params.id));
});

app.delete('/api/daily-workers/:id', authenticateToken, requireAdmin, async (req, res) => {
  await db.prepare('UPDATE daily_workers SET is_active=0 WHERE id=?').run(req.params.id);
  res.json({ success: true });
});
...
app.get('/api/daily-work-records', authenticateToken, async (req, res) => {
  ...
  if (date)
    ...
    rows = await db.prepare(`${base} WHERE dwr.approved = 1 ORDER BY dwr.work_date DESC, dwr.id DESC`).all();
  res.json(rows);
});
...
app.delete('/api/daily-work-records/:id', authenticateToken, requireAdmin, async (req, res) => {
  const row = await db.prepare('SELECT * FROM daily_work_records WHERE id = ?').get(req.params.id);
  if (!row) return res.status(404).json({ error: 'السجل غير موجود' });
  await db.prepare('DELETE FROM daily_work_records WHERE id = ?').run(req.params.id);
  res.json({ success: true });
});
...
/* ═══════════════════════════════════════════
   ATTENDANCE
═══════════════════════════════════════════ */
app.get('/api/attendance', authenticateToken, requireHostOrAdmin, async (req, res) => {
  const { date } = req.query;
  const rows = date
    ? await db.prepare('SELECT * FROM attendance WHERE attendance_date = ? ORDER BY id').all(date)
    : await db.prepare('SELECT * FROM attendance ORDER BY attendance_date DESC, id DESC LIMIT 300').all();
  res.json(rows);
});

app.post('/api/attendance', authenticateToken, requireHostOrAdmin, async (req, res) => {
  ...
  if (status === 'hourly') {
      const exists = await db.prepare('SELECT id FROM hourly_workers WHERE full_name = ?').get(worker_name);
      if (!exists) {
          await db.prepare('INSERT INTO hourly_workers (full_name, hourly_rate) VALUES (?, ?)').run(worker_name, Number(hourly_rate || 1.25));
      } else if (hourly_rate) {
          await db.prepare('UPDATE hourly_workers SET hourly_rate = ? WHERE id = ?').run(Number(hourly_rate), exists.id);
      }
  }

  const session = await activeSession();
  const result = await db.prepare('INSERT INTO attendance (worker_name, attendance_date, status, session_id, created_by) VALUES (?,?,?,?,?)')
    .run(worker_name, attendance_date, status, session?.id ?? null, req.user.id);
  res.status(201).json(await db.prepare('SELECT * FROM attendance WHERE id = ?').get(result.lastInsertRowid));
});

app.delete('/api/attendance/:id', authenticateToken, requireHostOrAdmin, async (req, res) => {
...
═══════════════════════════════════════════ */
app.get('/api/inspection-records', authenticateToken, async (req, res) => {
  const { date, session_id } = req.query;
  const session = await activeSession();
...
app.delete('/api/inspection-records/:id', authenticateToken, requireAdmin, async (req, res) => {
  await db.prepare('DELETE FROM inspection_records WHERE id = ?').run(req.params.id);
  res.json({ success: true });
});
...
app.delete('/api/inspection-targets/:id', authenticateToken, requireAdminOrInspectionCoordinator, async (req, res) => {
  await db.prepare('DELETE FROM inspection_targets WHERE id = ?').run(req.params.id);
  res.json({ success: true });
});
...
app.get('/api/users', authenticateToken, requireAdmin, async (req, res) => {
  res.json(await db.prepare('SELECT id, full_name, username, role, is_active, two_factor_enabled, created_at FROM users ORDER BY id').all());
});
...
app.post('/api/users', authenticateToken, requireAdmin, async (req, res) => {
  ...
  try {
    const result = await db.prepare('INSERT INTO users (full_name, username, password_hash, role, is_active) VALUES (?,?,?,?,1)')
      .run(full_name, username, bcrypt.hashSync(password, 10), role);
    res.status(201).json(await db.prepare('SELECT id, full_name, username, role, is_active, created_at FROM users WHERE id = ?').get(result.lastInsertRowid));
...
app.put('/api/users/:id', authenticateToken, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    res.json(await db.prepare('SELECT id, full_name, username, role, is_active, two_factor_enabled, created_at FROM users WHERE id = ?').get(req.params.id));
  } catch { res.status(400).json({ error: 'اسم المستخدم موجود مسبقاً' }); }
});

app.post('/api/users/:id/two-factor', authenticateToken, requireAdmin, async (req, res) => {
  const user = await db.prepare('SELECT * FROM users WHERE id = ?').get(req.params.id);
  if (!user) return res.status(404).json({ error: 'المستخدم غير موجود' });
  const secret = generateSecret();
  await db.prepare('UPDATE users SET two_factor_enabled=1, two_factor_secret=? WHERE id=?').run(secret, req.params.id);
  res.json({ two_factor_secret: secret });
});

app.delete('/api/users/:id/two-factor', authenticateToken, requireAdmin, async (req, res) => {
  const user = await db.prepare('SELECT * FROM users WHERE id = ?').get(req.params.id);
  if (!user) return res.status(404).json({ error: 'المستخدم غير موجود' });
  await db.prepare('UPDATE users SET two_factor_enabled=0, two_factor_secret=NULL WHERE id=?').run(req.params.id);
  res.json({ success: true });
});
...
app.delete('/api/users/:username', authenticateToken, requireAdmin, async (req, res) => {
  ...
  await db.prepare('DELETE FROM users WHERE username = ?').run(req.params.username);
  res.json({ success: true });
});
...
app.post('/api/approvals/:type/:id/approve', authenticateToken, requireAdmin, async (req, res) => {
  const { type, id } = req.params;
  if (type === 'usage') {
    await db.prepare('UPDATE category_usage SET approved = 1 WHERE id = ?').run(id);
  } else if (type === 'boxes') {
    await db.prepare('UPDATE box_allocations SET approved = 1 WHERE id = ?').run(id);
  } else if (type === 'daily') {
    await db.prepare('UPDATE daily_work_records SET approved = 1 WHERE id = ?').run(id);
  } else {
...
app.delete('/api/approvals/:type/:id/reject', authenticateToken, requireAdmin, async (req, res) => {
  const { type, id } = req.params;
  if (type === 'usage') {
    await db.prepare('DELETE FROM category_usage WHERE id = ?').run(id);
  } else if (type === 'boxes') {
    await db.prepare('DELETE FROM box_allocations WHERE id = ?').run(id);
  } else if (type === 'daily') {
    await db.prepare('DELETE FROM daily_work_records WHERE id = ?').run(id);
  } else {
    return res.status(400).json({ error: 'نوع غير مدعوم' });
  }
  res.json({ success: true });
...
app.get('/api/reports/daily', authenticateToken, async (req, res) => {
  const { date } = req.query;
  const usage   = await db.prepare('SELECT COALESCE(SUM(total_price),0) as v FROM category_usage WHERE entry_date=? AND approved = 1').get(date);
  const shifts  = await db.prepare('SELECT COALESCE(SUM(total_pay),0) as v FROM shift_records WHERE shift_date=?').get(date);
  const daily   = await db.prepare('SELECT COALESCE(SUM(total_pay),0) as v FROM daily_work_records WHERE work_date=? AND approved = 1').get(date);
  res.json({
    date,
